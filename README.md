...
- **`tests/`** — 1009 tests (unit, mocked, integration, smoke, post-run validation; 1009 passing)
...
- v1.0.112 (Summarizer): added topic-alignment guard — `_has_topic_overlap()` rejects batch/recovery summaries that share zero significant keywords with the headline. Topic-mismatched summaries fall back to `[Auto] <headline>`. Eliminates harness check 3.3 zero-overlap failures on live runs. 977/984 tests passing (7 pre-existing RSS dedup), config validate PASS, live smoke test WARN (zero FAILs).
