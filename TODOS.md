...
- [x] Start independent weather providers concurrently rather than waiting for NWS, then climate/rainfall, then lakes.
  - Completed v1.0.135. `fetch_weather()` launches NWS, climate normal, monthly rainfall, and lake collection together via single `asyncio.gather`. Deterministic 4-provider event-gated test added.
