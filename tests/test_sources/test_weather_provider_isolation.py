...
class TestProviderFailurePermutations(TestCase):
    ...
    async def _run(
        self, climate_result=None, rain_result=None, raise_climate=False, raise_rain=False
    ):
        if raise_climate:
    ...
    def test_climate_fail_rain_succeeds(self):
        result = asyncio.get_event_loop().run_until_complete(
            self._run(
                raise_climate=True,
                rain_result={"avg_monthly_rainfall": 3.2, "current_monthly_rainfall": 2.1},
            )
        )
...
class TestMergeWeatherData(TestCase):
    ...
    def test_era5_absent_no_fallback(self):
        ...
        result = merge_weather_data(
            forecast, None, {"avg_monthly_rainfall": 3.2, "current_monthly_rainfall": 2.1}, {}, []
        )
        self.assertEqual(result["station"]["avg_temp_today"], "Unavailable")

    def test_rainfall_formatting(self):
        result = merge_weather_data(
            [], 91, {"avg_monthly_rainfall": 5.1, "current_monthly_rainfall": 3.2}, {}, []
        )
    ...
    def test_equal_rainfall_suppressed_and_invalid_payload(self):
        result = merge_weather_data(
            [], None, {"avg_monthly_rainfall": 4.2, "current_monthly_rainfall": 4.2}, {}, []
        )
        self.assertEqual(result["station"]["current_monthly_rainfall"], "Unavailable")
    ...
    def test_lake_ordering_preserved(self):
        ...
        result = merge_weather_data(
            [], 91, {"avg_monthly_rainfall": 3.2, "current_monthly_rainfall": 2.1}, lakes, []
        )
        self.assertIn("lake_crawford", result["lakes"])
    ...
    def test_errors_and_all_unavailable(self):
        ...
        result2 = merge_weather_data(
            [], None, {"avg_monthly_rainfall": None, "current_monthly_rainfall": None}, {}, []
        )
