...
class TestStartupSequence:
    ...
    def test_startup_sequence_reaches_phase4(self):
        ...
        from daily_brief.config import LOG_DIR as CFG_LOG, NEWS_DIR as CFG_NEWS, CATEGORIES
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch.object(pipeline_mod, "fetch_weather", new_callable=AsyncMock, return_value={
                    "forecast": [{"name": "Night", "temperature": "70\u00b0F", "winds": "Calm", "short_forecast": "Clear skies"}],
                    "station": {"avg_temp_today": "74\u00b0F", "avg_monthly_rainfall": "2.1\"", "current_monthly_rainfall": "1.8\""},
                    "lakes": {},
                }),
                patch.object(pipeline_mod, "fetch_and_dedup", new_callable=AsyncMock, return_value=(
                    [("StartupTestStory", "https://example.com/1", "Test snippet", None, "Artificial Intelligence")],
                    {"total_after": 1},
                )),
                patch.object(pipeline_mod, "llm_batch_summarize_all", new_callable=AsyncMock, return_value=None),
                patch.object(pipeline_mod, "validate_report", return_value=(False, ["test validation"])),
            ):
                asyncio.get_event_loop().run_until_complete(pipeline_mod.main())

            # Log file created
            log_files = [f for f in os.listdir(tmp_log) if f.startswith("run_log_") and f.endswith(".md")]
            ...
            # Report file created
            report_files = [f for f in os.listdir(tmp_news) if f.startswith("DailyBrief-") and f.endswith(".md")]
